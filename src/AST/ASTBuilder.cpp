...
antlrcpp::Any ASTBuilder::visitType(StaticScriptParser::TypeContext *ctx) {
    ...
    } else if (ctx->socketType())  {
        type = visitSocketType(ctx->socketType()).as<SharedPtr<SocketType>>();
    } else {
...
antlrcpp::Any ASTBuilder::visitSocketType(StaticScriptParser::SocketTypeContext *ctx) {
    const SharedPtr<SocketType> &socketType = makeShared<SocketType>();
    return socketType;
}
...
antlrcpp::Any ASTBuilder::visitExpression(StaticScriptParser::ExpressionContext *ctx) {
    ...
    } else if (auto ternaryExprCtx = dynamic_cast<StaticScriptParser::TernaryExprContext *>(ctx)) {
        exprAny = visitTernaryExpr(ternaryExprCtx);
        expr = exprAny.as<SharedPtr<TernaryOperatorExprNode>>();
...
antlrcpp::Any ASTBuilder::visitTernaryExpr(StaticScriptParser::TernaryExprContext *ctx) {
    SharedPtr<ExprNode> bhs = visitExpression(ctx->expression(0));
    SharedPtr<ExprNode> lhs = visitExpression(ctx->expression(1));
    SharedPtr<ExprNode> rhs = visitExpression(ctx->expression(2));
    return makeShared<TernaryOperatorExprNode>(bhs, lhs, rhs);
}
...
antlrcpp::Any ASTBuilder::visitLiteral(StaticScriptParser::LiteralContext *ctx) {
    ...
    if (ctx->BooleanLiteral()) {
        ...
        } /*else if (ctx->DecimalIntegerWithComma()) {
            base = 10;
            String tempStr = ctx->DecimalIntegerWithComma()->getText();
            tempStr.erase(std::remove(tempStr.begin(), tempStr.end(), ','), tempStr.end()); // 删除掉书写代码上的逗号位数标记。
            literalStr = tempStr;
