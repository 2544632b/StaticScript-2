...
void SemanticValidator::visit(const SharedPtr<IfStmtNode> &ifStmt) {
    ...
    if (ifStmt->condition == nullptr) {
        reportSemanticError("A if condition is empty");
    }
    else if (!conditionType->isBoolean()) {
        reportSemanticError("A if condition is not boolean");
    }
...
void SemanticValidator::visit(const SharedPtr<WhileStmtNode> &whileStmt) {
    const SharedPtr<Type> conditionType = whileStmt->condition->type;
    if (whileStmt->condition == nullptr) {
        reportSemanticError("A while condition is empty");
    }
    else if (!conditionType->isBoolean()) {
        reportSemanticError("A while condition is not boolean");
    }
}
