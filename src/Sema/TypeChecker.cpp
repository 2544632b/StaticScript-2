...
void TypeChecker::visit(const SharedPtr<VarDeclNode> &varDecl) {
    ...
    if (varInitVal) {
        ...
        } else if (varDeclType->isSocket()) {
            reportTypeError("You should create a object for this: " + varDecl->name);
        }
        varInitVal = initVal;
    }
}
...
void TypeChecker::visit(const SharedPtr<ArraySubscriptExprNode> &asExpr) {
    ...
    if (!asExpr->baseExpr->type->isArray() && !asExpr->baseExpr->type->isString()) {
    ...
    else if (asExpr->baseExpr->type->isString()) {
        asExpr->type = asExpr->baseExpr->type;
    }
}
...
// Note: 2 or higher the count of returns should be same.
