...
void IRGenerator::visit(const SharedPtr<ModuleNode> &module) {
    ...
    // llvmIRBuilder.CreateCall(BuiltinLoop::defaultLoopFunc);

    ASTVisitor::visit(module);
    setFuncInsertPoint(mainFn);
    
    // llvmIRBuilder.CreateCall(BuiltinLoop::loopSetupFunc);
...
LLVMType *IRGenerator::getType(const SharedPtr<Type> &inputType) {
    ...
    if (inputType) {
        ...
        } else if (inputType->isSocket()) {
            type = BuiltinSocket::type;
        }
    }
    return type;
}
