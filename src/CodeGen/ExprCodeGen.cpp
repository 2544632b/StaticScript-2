...
#define GenerateBasicArray(LiteralType, UpperCaseType, AllocaType) \
    ...
    if (std::string(#UpperCaseType) == "Integer") { \
        elementType = llvmIRBuilder.getInt64Ty(); \
    } \
    else if (std::string(#UpperCaseType) == "Boolean") { \
        elementType = llvmIRBuilder.getInt1Ty(); \
    } \
    else if (std::string(#UpperCaseType) == "Float") { \
        elementType = llvmIRBuilder.getDoubleTy(); \
    } \
    else { \
        reportCodeGenError("Please use the complex type!"); \
    } \
    \
    LLVMType *arrayType = llvm::ArrayType::get(elementType, elementSize); \
    LLVMValue *literalListAlloca = llvmIRBuilder.CreateAlloca( \
            /*llvm::ArrayType::get(llvmIRBuilder.get##AllocaType##Ty(), elementSize)*/ arrayType \
    ); \
...
#define GenerateComplexArray(UpperCaseType) \
    LLVMType *elementType = Builtin##UpperCaseType::type; \
    \
    LLVMType *arrayType = llvm::ArrayType::get(elementType, elementSize); \
    LLVMValue *literalListAlloca = llvmIRBuilder.CreateAlloca( \
            /*llvm::ArrayType::get(Builtin##UpperCaseType::type, elementSize)*/ arrayType \
    ); \
...
void IRGenerator::visit(const SharedPtr<ArrayLiteralExprNode> &arrayLiteralExpr) {
    ...
    if (arrayLiteralExpr->elements.empty()) {
        ...
        } else if (elementType->isSocket()) {
            reportTypeError("Unsupported type socket!");
        }
    ...
    LLVMValue *errorLoad = llvmIRBuilder.CreateLoad(BuiltinError::type, errorAlloca);
    #else 
...
void IRGenerator::visit(const SharedPtr<IdentifierExprNode> &varExpr) {
    // 按右值处理
    #if LLVM_VERSION_MAJOR >= 14
    varExpr->code = llvmIRBuilder.CreateLoad(IRGenerator::getType(varExpr->type), varExpr->refVarDecl->code);
    #else
...
LLVMValue* generateNullishCoalescing(LLVMValue* lhs, LLVMValue* rhs, llvm::IRBuilder<> &builder) {
    ...
    } else if (lhs->getType()->isPointerTy()) { // P.S.: 事实上上面的struct一般用不到，真正传入的都是指针，但是在语言层上指针不可见。
...
void IRGenerator::visit(const SharedPtr<BinaryOperatorExprNode> &bopExpr) {
    ...
    switch (bopCode) {
        ...
        case StaticScriptLexer::PlusAssign: {
            // 判定：如果两边有一边是一个字符串，那就将其连接起来。
            if (type->isString()) { // 左边是一个字符串 
            ...
            } else if (rtype->isString()) {  // 右边是一个字符串
...
void IRGenerator::visit(const SharedPtr<ArraySubscriptExprNode> &asExpr) {
    ...
    for (const SharedPtr<ExprNode> &indexExpr: asExpr->indexExprs) {
        ...
        if(iterType->isArray()) {
            ...
            } else if (iterEleType->isSocket()) {
                reportCodeGenError("You can't get a socket from a array!");
            }
            ...
            getFunc = BuiltinString::getStringCharacterFunc;
            iterCode = llvmIRBuilder.CreateCall(getFunc, Vector<LLVMValue *>{iterCode, indexExpr->code, errorAlloca});
        }
